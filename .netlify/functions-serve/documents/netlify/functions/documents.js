// netlify/functions/documents.cjs
var documents = [];
exports.handler = async function(event, context) {
  ...
  if (path.startsWith("/documents")) {
    if (method === "POST") {
      const newDocument = { id: Date.now(), createdAt: /* @__PURE__ */ new Date(), ...JSON.parse(event.body) };
      ...
      if (searchTerm) {
        ...
        filteredDocuments = filteredDocuments.filter(
          (doc) => doc.clientName.toLowerCase().includes(lowerCaseSearchTerm) || doc.documentNumber.toLowerCase().includes(lowerCaseSearchTerm)
        );
      }
      return {
        statusCode: 200,
